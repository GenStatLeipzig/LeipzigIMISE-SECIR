...
step_sm_hook_jeeves_nonmem<-function(optim_options,ind_par_loop,xtot,main_struct,label_opt, opt_mod,ind_indiv)
{
  ...
  {    #if positive and negative perturbations lay in allowed interval
    ...
    #cand_sim <- funevalTogetherLLadvThromboResest(x0,main_struct,opt_mod);
      ...
      cand_sim <- GoalFuncComp(xtot=xtot,main_struct,opt_mod,label_opt,nLL_indiv = nLL_indiv_min,ind_indiv1 = ind_indiv)
     # cand_sim <-             GoalFuncComp(xtot=xtot,main_struct,opt_mod=opt_mod,label_opt=label_opt,
...
step_me_hook_jeeves_nonmem_indiv<-function(xtot,main_struct,ind_indiv)
{
  optim_options<-main_struct$optim_options
  ...
  label_opt <- 'indiv'
      ...
      {#length(xtot)
        ...
        #temp_struct <-                step_me_hook_jeeves_nonmem_indiv(xtot=xtot,main_struct=main_struct,ind_indiv)
        temp_struct <- step_sm_hook_jeeves_nonmem(optim_options,ind_par_loop,xtot=xtot,main_struct,label_opt='indiv',opt_mod=0,ind_indiv)
        ...
        {
          print(c(ind_par_loop,optim_options$nLL_tot_min))###in the big loop
  ...
  #xtot<-EstimDistribIndivPar(xtot,main_struct)##not relevant for Covid!!! 28.05.2021
  output_struct<-list()
