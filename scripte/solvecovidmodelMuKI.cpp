...
NumericMatrix GenerateUpdateTreatStructDir( integral_par param_, DoubleVector blockeff_vec,DoubleVector unlockeff_vec,
                                            IntegerVector Lock_vec, IntegerVector Unlock_vec, int num_steps);
NumericMatrix  GenerateTreatStructKIDir(integral_par param_, DoubleVector treat_vec,  IntegerVector LockUnlock_vec, int num_steps );


//[[Rcpp::export]]
NumericMatrix solvecovidmodelMuKI( DoubleVector parval,CharacterVector parnames, int numpar, DoubleVector u0,
                                 ...
                                 DoubleVector treat_vec,//DoubleVector unlockeff_vec,
                                 IntegerVector LockUnlock_vec,  int treatpardir)//NumericMatrix subintervals, NumericMatrix subintervals
{
  //u0 is the initial value!
    // int num_steps, int numvar
  // cout<<"l 30, start, compind= "<<endl;  
  // cout<<"l 29, LockUnlock_vec(0,1,2,3)"<<LockUnlock_vec(0)<<", "<<LockUnlock_vec(1)<<", "<<LockUnlock_vec(2)<<", "<<LockUnlock_vec(3)<<endl;
  ...
  }else if(treatpardir==2)
  { */
      treat_arr = GenerateTreatStructKIDir(par00, treat_vec,  LockUnlock_vec, num_steps );
      //}
  ...
  //cout<<"l69"<<endl;///
    ...
    {  
      intervals_death(ind_e+1) = std::min(num_steps,death_int_round(ind_e));  //intervals_death(ind_e)+..death_int_round is death_date
    } 
  for(int ind_e = 0; ind_e <(crit_exp_n-2); ind_e++)
  {  
    intervals_crit(ind_e+1) = std::min(num_steps,crit_int_round(ind_e)); //intervals_crit(ind_e)+
  }
  ...
  */
  ...
  ///
    for(int ind_e=0; ind_e <(death_exp_n-1);ind_e++)
    {
      if(ind_e==0)
      {
        ...
        {
          pdeath_v(ind_int) = std::min(0.66,pdeath0);//1;
        }
        ...
        for(int ind_int=(intervals_death(ind_e)-1); ind_int <(intervals_death(ind_e+1));ind_int++)
        {
          pdeath_v(ind_int) = std::min(0.66,pdeath0*death_exp(ind_e-1));
        }
      }
  ...
  sim_arr(0,numvar) = std::max(0.0,u1(compartments00.ISc_ind(0))+u1(compartments00.ISMuc_ind(0)));
  DICplus(0) = 0;
  ...
  for(int indi =1; indi<num_steps; indi++)
  {
    ...
    if(indi<par00.influx_start)
    {
      ...
      { 
        par00.influx = DelayEff(indi,influx0,0,LockUnlock_vec(0),par00.delaylock);//Lock_vec instead par00.Lock1!!!
      }
    }
    ...
    nextav_= covidmodelMutant(u1, par00, compartments00,  compnames, compl_varstart);
    double help_v =  pIA1IS1*par00.rb4*(std::max(0.0,u1(compartments00.IAc_ind(0)))+std::max(0.0,u1(compartments00.IAMuc_ind(0))));//previous step
    for(int indl =0;  indl < u0.length(); indl++)
    {
      sim_arr(indi, indl) = u1(indl)+par00.delt*nextav_(indl);//par00.delt*nextav_(indl)  Not necessary yet!!!!
       // u1(indl) = sim_arr(indi, indl);
    } 
     DICplus(indi)=help_v+(1-pIA1IS1)*par00.spdeath*(std::max(0.0,u1(compartments00.ISc_ind(1)))+std::max(0.0,u1(compartments00.ISMuc_ind(1))));
    sim_arr(indi,numvar) = sim_arr(indi-1,numvar)+DICplus(indi-1);
    if(indi == (par00.date_mu-1))//(indi >= (par00.date_mu-1)) //par00.date_mu+1!!!?? par00.date_mu+1 or par00.date_mu - dependent on whther this parameter is estimated???
    {
     /* sim_arr(indi,compartments00.IAMuc_ind(0)) = sim_arr(indi,compartments00.IAMuc_ind(0)) + par00.Iamuinit/7;
      sim_arr(indi,compartments00.IAMuc_ind(1)) = sim_arr(indi,compartments00.IAMuc_ind(1)) + par00.Iamuinit/7;
      sim_arr(indi,compartments00.IAMuc_ind(2)) = sim_arr(indi,compartments00.IAMuc_ind(2)) + par00.Iamuinit/7;
      sim_arr(indi,compartments00.ISMuc_ind(0)) = sim_arr(indi,compartments00.ISMuc_ind(0)) + par00.Iamuinit/7;
      sim_arr(indi,compartments00.ISMuc_ind(1)) = sim_arr(indi,compartments00.ISMuc_ind(1)) + par00.Iamuinit/7;
      sim_arr(indi,compartments00.ISMuc_ind(2)) = sim_arr(indi,compartments00.ISMuc_ind(2)) + par00.Iamuinit/7;*/
//sim_arr(indi,compartments00.EMuc_ind) = sim_arr(indi,compartments00.EMuc_ind) + par00.Iamuinit/7;
     sim_arr(indi,compartments00.IAMuc_ind(0)) = sim_arr(indi,compartments00.IAc_ind(0))*par00.Iamuinit;
      sim_arr(indi,compartments00.IAMuc_ind(1)) = sim_arr(indi,compartments00.IAc_ind(1))*par00.Iamuinit;
      sim_arr(indi,compartments00.IAMuc_ind(2)) = sim_arr(indi,compartments00.IAc_ind(2))*par00.Iamuinit;
      sim_arr(indi,compartments00.ISMuc_ind(0)) = sim_arr(indi,compartments00.ISc_ind(0))*par00.Iamuinit;
      sim_arr(indi,compartments00.ISMuc_ind(1)) = sim_arr(indi,compartments00.ISc_ind(1))*par00.Iamuinit;
      sim_arr(indi,compartments00.ISMuc_ind(2)) = sim_arr(indi,compartments00.ISc_ind(2))*par00.Iamuinit;
      sim_arr(indi,compartments00.EMuc_ind) = sim_arr(indi,compartments00.Ec_ind) * par00.Iamuinit;
      //cout<<"l178, indi = "<< indi <<", u1(compartments00.EMuc_ind) = "<<u1(compartments00.EMuc_ind)<<", sim_arr(indi-1,compartments00.EMuc_ind) = "<<sim_arr(indi,compartments00.EMuc_ind)<<endl;
      //cout<<"l171, indi = "<< indi <<endl;
    }
    for(int indl =0;  indl < u0.length(); indl++)
    {
//sim_arr(indi, indl) = u1(indl)+par00.delt*nextav_(indl);//par00.delt*nextav_(indl)  Not necessary yet!!!!
  ...
  NumericMatrix  GenerateTreatStructKIDir(integral_par param_, DoubleVector treat_vec,  IntegerVector LockUnlock_vec, int num_steps )
{
  {
    //int num_steps =  param_.num_steps;
    int num_treats =  param_.num_locks+param_.num_unlocks;//!!! after decide on num_treat!!!//blockeff_vec.length();//param_.num_locks; 
    //int num_unlocks =  param_.num_unlocks;//unlockeff_vec.length();
    ...
    for(int indi = 0; indi< num_steps; indi++)
    {
      ...
      {
        help_val = DelayEff(indi,prev_val, treat_vec(ind_treat), LockUnlock_vec(ind_treat),param_.delaylock);
        treat_arr(indi,0)= treat_arr(0,0)*help_val;
        treat_arr(indi,1)= treat_arr(0,1)*help_val;
        //cout<<"l 376, indi = "<<indi<<"ind_treat = "<<ind_treat<<", LockUnlock_vec(ind_treat) = "<<LockUnlock_vec(ind_treat)<<", help_val = "<<help_val<<", treat_arr(indi,0) = "<<treat_arr(indi,0)<<endl;
        if(indi == (LockUnlock_vec(ind_treat)+param_.delaylock))
        {
          prev_val = treat_vec(ind_treat);
          if(ind_treat<(num_treats-1))
          {
